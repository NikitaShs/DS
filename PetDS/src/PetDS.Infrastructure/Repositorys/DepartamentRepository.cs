...
public class DepartamentRepository : IDepartamentRepository
{
    ...
    public async Task<Result<int, Errors>> UpdateDepartamentFullHierahiById(Guid id, Guid? parentId,
        CancellationToken cancellationToken)
    {
        DbConnection connection = _applicationDbContext.Database.GetDbConnection();
        ...
        int res = await connection.ExecuteAsync(sql, new { parentId, id });

        _logger.LogInformation($"затронуто {res}");

        if (res >= 0)
        {
            DepartamentId? updated = await _applicationDbContext.Departaments
    ...
    public async Task<Result<bool, Errors>> CheckingDepartamentExistence(DepartamentId departamentId,
        CancellationToken cancellationToken) =>
        await _applicationDbContext.Departaments.AnyAsync(q => q.Id == departamentId);
    ...
    public async Task<Result<bool, Errors>> SoftDeleteDept(Guid departamentId, CancellationToken cancellationToken)
    {
        ...
        string sql = """
                     ...
                     """;
        DateTime delTime = DateTime.UtcNow.AddYears(1);
